...
public class HomePresenter extends BasePresenter<HomeContract.View, HomeContract.Model> implements HomeContract.presenter {
    ...
    @Override
    public void request() {
        mModel.request().subscribe(new ProgressDialogSubscribe<Reply<List<User>>>(mActivity) {
            @Override
    ...
    @Override
    public void getUser() {
        mModel.getUser(mView.getSelectUser()).subscribe(new ProgressDialogSubscribe<User>(mActivity) {
            @Override
